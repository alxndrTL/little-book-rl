"""
Simple Policy Gradient (SPG) as described in the book.
...
def discounted_return(rewards, gamma):
    """G(tau) = sum_{t=0}^{T-1} gamma^t r_t"""
    G = 0.0
...
if __name__ == "__main__":
    ...
    while global_step < args.total_timesteps:
            ...
            for i in range(args.num_envs):
                ...
                if done[i]:
                    trajectories.append({
                        ...
                        "G": discounted_return(ep_rewards[i], args.gamma),
        ...
        # =====================================================
        # 2. SPG gradient step
        #    g_hat = (1/N) * sum_i G_i * sum_t grad log pi(a_t^i | s_t^i)
        #    minimize  L(theta) = -(1/N) * sum_i G_i * sum_t log pi(a_t^i | s_t^i)
        # =====================================================
        batch_obs = torch.cat([t["obs"] for t in trajectories])
        batch_actions = torch.cat([t["actions"] for t in trajectories])
        # Each timestep within trajectory i is weighted by the same scalar G_i
        batch_G = torch.cat([
            torch.full((t["length"],), t["G"], dtype=torch.float32)
        ...
        mean_disc = float(np.mean([t["G"] for t in trajectories]))
